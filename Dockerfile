...
RUN git clone --branch 4.6-stable https://github.com/ntop/nDPI.git nDPI && git clone --branch 5.6-stable https://github.com/ntop/ntopng.git ntopng && git clone https://github.com/synfinatic/netflow2ng netflow2ng
COPY Makefile . 
COPY run.sh .
RUN make -B -j8 && mv /ntop/netflow2ng/dist/netflow2ng-0.0.4 /ntop/netflow2ng-0.0.4 && rm -r /ntop/netflow2ng && chmod +x ntopng/ntopng && chmod +x run.sh
EXPOSE 3000/tcp
EXPOSE 2055/udp
